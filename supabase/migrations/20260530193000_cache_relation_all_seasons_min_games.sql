...
create or replace function private.calculate_relation_all_seasons_min_games()
returns integer
language plpgsql
security definer
...
as $$
declare
  v_completed_season_count integer := 0;
  v_match_count integer := 0;
  v_pair_count integer := 0;
  v_pair_games_p40 numeric := null;
  v_threshold integer := 3;
begin
  select
    count(distinct m.season_id)::integer,
    count(*)::integer
  into v_completed_season_count, v_match_count
  from public.matches m
  where m.status = 'approved'
    and m.winner_side is not null;

  with relation_games as (
    select
      'teammate'::text as relation_type,
      stats.player_id,
      stats.related_player_id,
      sum(stats.games)::numeric as games
    from public.v_player_teammate_stats stats
    group by stats.player_id, stats.related_player_id

    union all

    select
      'opponent'::text as relation_type,
      stats.player_id,
      stats.related_player_id,
      sum(stats.games)::numeric as games
    from public.v_player_opponent_stats stats
    group by stats.player_id, stats.related_player_id
  )
  select
    count(*)::integer,
    percentile_cont(0.4) within group (order by relation_games.games::double precision)
  into v_pair_count, v_pair_games_p40
  from relation_games
  where relation_games.games >= 3;

  v_threshold := greatest(3, ceil(coalesce(v_pair_games_p40, 3))::integer);

  if v_completed_season_count > 1 then
    v_threshold := greatest(v_threshold, 5);
  end if;

  v_threshold := greatest(
    v_threshold,
    3 + floor(greatest(v_completed_season_count - 1, 0)::numeric / 3)::integer
  );

  v_threshold := greatest(
    v_threshold,
    3 + floor(sqrt(greatest(v_match_count, 0)::numeric) / 6)::integer
  );

  return greatest(3, least(v_threshold, 18));
end;
$$;

comment on function private.calculate_relation_all_seasons_min_games()
  is 'Calculates the cached all-season relationship minimum sample size from historical relationship pair counts. Intended to run during monthly season rollover.';

update public.seasons
set rule_config = jsonb_set(
      coalesce(rule_config, '{}'::jsonb),
      '{relation_all_seasons_min_games}',
      to_jsonb(private.calculate_relation_all_seasons_min_games()),
      true
    ),
    updated_at = timezone('utc', now())
where status in ('draft', 'active')
  and coalesce(rule_config ->> 'relation_all_seasons_min_games', '') !~ '^\d+$';
...
begin
  ...
  v_cutoff := timezone(
    'utc',
    ((v_season_month_start::timestamp + interval '1 month' + interval '6 hours') at time zone 'Asia/Shanghai')
  );
...
comment on function public.confirm_season_rollover(uuid)
  is 'Closes the current editable season (draft or active). Rollover opens at 06:00 Asia/Shanghai on the first day of the next season month; season managers may finalize immediately, while scorekeepers still require two confirmations. It also caches the all-season relationship minimum sample size for the next season.';
