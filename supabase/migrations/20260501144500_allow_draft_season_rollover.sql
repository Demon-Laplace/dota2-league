...
comment on function public.confirm_season_rollover(uuid)
  is 'Closes the current editable season (draft or active), lets admins finalize immediately, skips retaining empty seasons, and opens the next season while keeping closed seasons read-only in Supabase until exported.';
