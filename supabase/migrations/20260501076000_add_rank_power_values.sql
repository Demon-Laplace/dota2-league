...
begin
  ...
  if p_power_value is not null and (p_power_value < 0 or p_power_value > 9) then
    raise exception 'power_value must be between 0 and 9.'
      using errcode = '22023';
  end if;
...
comment on function public.set_season_rank_profile(uuid, integer, text, integer) is 'Sets or clears the display label and single-digit power value for a season rank bucket.';
