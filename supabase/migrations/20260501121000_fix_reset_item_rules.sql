...
begin
  ...
  loop
    loop
      ...
      values (
        ...
        case
          when v_stack_pair.score_delta_multiplier = -99 then 0
    ...
    loop
      ...
      values (
        ...
        case
          when v_single_effect.score_delta_multiplier = -99 then 0
  ...
  loop
    ...
    if v_applied_group.applied_multiplier = -99 then
...
comment on function private.apply_match_double_downs(uuid, jsonb, uuid) is
  'Applies configured item score multipliers for match item effects. Reset multiplier -99 is only allowed on positive match-result deltas when the player''s pre-match competitive total is below the season initial score.';
