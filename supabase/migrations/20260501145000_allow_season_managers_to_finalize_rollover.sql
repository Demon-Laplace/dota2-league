...
comment on function public.confirm_season_rollover(uuid)
  is 'Closes the current editable season (draft or active). Season managers may finalize immediately; scorekeepers still require two confirmations. Closed seasons stay read-only in Supabase until exported.';
