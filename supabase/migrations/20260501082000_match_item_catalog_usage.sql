...
comment on column public.item_catalog.config is 'Extensible item definition. Current front-end stores donation_amount, operator_roles, match_scope, and match_icon for catalog management and match registration.';
...
begin
  ...
  where iu.item_instance_id = ii.id
    ...
    and coalesce(ii.metadata ->> 'source_kind', '') = 'match_double_down';

  delete from private.item_instances ii
  where coalesce(ii.metadata ->> 'source_kind', '') = 'match_double_down'
    and ii.metadata ->> 'match_id' = p_match_id::text
    and not exists (
      select 1
