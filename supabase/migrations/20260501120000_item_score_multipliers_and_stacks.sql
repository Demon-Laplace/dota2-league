...
alter table public.item_catalog
  add column if not exists score_delta_multiplier numeric(10, 2) not null default 0;

comment on column public.item_catalog.score_delta_multiplier is
  'Final score multiplier applied to the affected player''s match_result points_delta. 0 means the item does not change points; 2 means the final score becomes 2x.';

create table if not exists public.item_catalog_score_stacks (
  id uuid primary key default gen_random_uuid(),
  item_catalog_id_low uuid not null references public.item_catalog(id) on delete cascade,
  item_catalog_id_high uuid not null references public.item_catalog(id) on delete cascade,
  score_delta_multiplier numeric(10, 2) not null default 0,
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  constraint item_catalog_score_stacks_pair_order_chk
    check (item_catalog_id_low < item_catalog_id_high),
  constraint item_catalog_score_stacks_distinct_pair_chk
    check (item_catalog_id_low <> item_catalog_id_high),
  constraint item_catalog_score_stacks_pair_key
    unique (item_catalog_id_low, item_catalog_id_high)
);

comment on table public.item_catalog_score_stacks is
  'Pairwise stack rules for score-changing item effects. When both items affect the same player in one match, the configured stack multiplier overrides the two individual multipliers for that pair.';

comment on column public.item_catalog_score_stacks.score_delta_multiplier is
  'Final score multiplier applied when the two item effects stack on the same player. This replaces the pair''s individual multipliers for that stack.';

drop trigger if exists item_catalog_score_stacks_set_updated_at on public.item_catalog_score_stacks;
create trigger item_catalog_score_stacks_set_updated_at
  before update on public.item_catalog_score_stacks
  for each row execute function public.tg_set_updated_at();

alter table public.item_catalog_score_stacks enable row level security;

drop policy if exists item_catalog_score_stacks_select_authenticated on public.item_catalog_score_stacks;
create policy item_catalog_score_stacks_select_authenticated
  on public.item_catalog_score_stacks
  for select
  to authenticated
  using (true);

drop policy if exists item_catalog_score_stacks_select_anon on public.item_catalog_score_stacks;
create policy item_catalog_score_stacks_select_anon
  on public.item_catalog_score_stacks
  for select
  to anon
  using (true);

drop policy if exists item_catalog_score_stacks_write_scorekeeper on public.item_catalog_score_stacks;
create policy item_catalog_score_stacks_write_scorekeeper
  on public.item_catalog_score_stacks
  for all
  to authenticated
...
grant select on public.item_catalog_score_stacks to anon, authenticated;
grant insert, update, delete on public.item_catalog_score_stacks to authenticated;
...
begin
  ...
  loop
    ...
    if v_item_catalog_id is not null then
      select ic.name, coalesce(ic.score_delta_multiplier, 0)
      into v_item_catalog_name, v_item_score_delta_multiplier
  ...
  loop
    loop
      ...
      order by abs(stacks.score_delta_multiplier) desc,
      ...
      values (
        ...
        case
          when v_stack_pair.score_delta_multiplier = -99 then v_stack_pair.base_points_delta * v_stack_pair.score_delta_multiplier
    ...
    loop
      ...
      values (
        ...
        case
          when v_single_effect.score_delta_multiplier = -99 then v_single_effect.base_points_delta * v_single_effect.score_delta_multiplier
  ...
  loop
    ...
    if v_applied_group.applied_multiplier = -99 and v_applied_group.base_points_delta > 0 then
      select coalesce(
        v_initial_score + sum(
          case
    ...
    if v_applied_group.applied_points_delta <> 0 then
      ...
      values (
        ...
        jsonb_build_object(
          ...
          'reset_to_initial_win_score', (v_applied_group.applied_multiplier = -99 and v_applied_group.base_points_delta > 0),
...
comment on function private.apply_match_double_downs(uuid, jsonb, uuid) is
  'Applies configured item score multipliers for match item effects. Pair stack rules override the paired items'' individual multipliers on the same player, while legacy null-catalog entries keep the historical extra x1 behaviour.';
