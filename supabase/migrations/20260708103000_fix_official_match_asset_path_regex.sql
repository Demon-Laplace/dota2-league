...
alter table public.official_match_assets
  drop constraint if exists official_match_assets_storage_path_check;

alter table public.official_match_assets
  add constraint official_match_assets_storage_path_check
    check (storage_path is null or storage_path ~ '^[0-9]{4}-[0-9]{2}/[0-9]+\.(jpg|jpeg|png|webp)$');

alter table public.official_match_snapshots
  drop constraint if exists official_match_snapshots_screenshot_path_check;

alter table public.official_match_snapshots
  add constraint official_match_snapshots_screenshot_path_check
