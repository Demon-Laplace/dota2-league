...
alter table public.item_catalog
  add column if not exists score_delta_special text;

alter table public.item_catalog_score_stacks
  add column if not exists score_delta_special text;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'item_catalog_score_delta_special_check'
  ) then
    alter table public.item_catalog
      add constraint item_catalog_score_delta_special_check
      check (score_delta_special is null or score_delta_special = '@');
  end if;
end
$$;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'item_catalog_score_stacks_score_delta_special_check'
  ) then
    alter table public.item_catalog_score_stacks
      add constraint item_catalog_score_stacks_score_delta_special_check
      check (score_delta_special is null or score_delta_special = '@');
  end if;
end
$$;

comment on column public.item_catalog.score_delta_special is
  'Special score-effect token. @ means reset the affected player''s competitive score back to the season initial score when eligible.';

comment on column public.item_catalog_score_stacks.score_delta_special is
  'Special stack-effect token. @ means reset the affected player''s competitive score back to the season initial score when eligible.';

update public.item_catalog
set
  score_delta_special = '@',
  score_delta_multiplier = 0
where score_delta_special is null
  and score_delta_multiplier = -99;

update public.item_catalog_score_stacks
set
  score_delta_special = '@',
  score_delta_multiplier = 0
where score_delta_special is null
...
comment on function private.apply_match_double_downs(uuid, jsonb, uuid) is
  'Applies configured item score multipliers for match item effects. Special token @ resets the affected player''s competitive total back to the season initial score when eligible; numeric multipliers, including -99, are treated as ordinary multipliers.';
