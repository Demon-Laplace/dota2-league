...
comment on function public.get_admin_system_usage() is 'Admin-only usage snapshot for current database size and Supabase Storage object size. The database quota reflects the visible database-only limit and excludes WAL/system disk estimates.';
